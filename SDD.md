...
**Observação:** quatro mensagens reais no Telegram (14/09/2026, 18h17–18h18), cada uma classificada corretamente pelo agente de ingestão — e mesmo assim o item preparado ("hambúrguer de patinho") nunca entrou no estoque. O guardrail de porcionamento ("nunca chutar, sempre perguntar quando falta número", §13.2) disparou como projetado, e ainda assim o ciclo pergunta→resposta nunca se resolveu: nenhum erro, nenhum log, nenhuma linha no feed. Evidência completa em `docs/apoio/evidencia-porcionamento-patinho.md` (os quatro traces preservados).
**Diagnóstico:** rodando o framework de diagnóstico do próprio produto (prompt, dados ou modelo?) contra as quatro chamadas, nenhuma das três categorias explica a falha — prompt correto, dados claros, modelo certo nas quatro. A causa é uma quarta categoria, arquitetural: `ingerirRelato` recebe só o texto da mensagem atual, sem histórico, e o ramo `porcionamento` de `aplicarIntencao` montava a `pergunta` e retornava **sem persistir nada** (o insert final só acontecia quando não havia pergunta). No momento exato em que o produto pedia uma informação, ele não registrava que a tinha pedido — então a resposta seguinte, que naturalmente não repete o nome do prato, era classificada do zero. Contraste com o único outro guardrail que pergunta e funciona: o fluxo de orçamento (`aguardando_categoria` + `callback_data: bg:categoria:${pensamentoId}`, `telegram.js`) persiste a pendência antes de perguntar, e a resposta chega ancorada nela pelo botão.
**Restrição derivada:** toda pergunta de guardrail cuja resposta pode chegar em mensagem separada precisa **persistir um estado de "pendente" antes de perguntar** — nunca confiar em memória de conversa implícita, nem supor que a próxima mensagem vai repetir o contexto que a pergunta já tinha. "Perguntar em vez de chutar" só é seguro emparelhado com "guardar a pergunta em algum lugar que a próxima mensagem consulta antes de classificar do zero". E persistir não basta: **toda pendência precisa de um destino final visível** — resolvida (`completo`) ou **expirada com aviso explícito** —, porque pendência que fica muda pra sempre é a mesma falha silenciosa que o §8 proíbe e que o §11.6 já documentou noutro contexto. Corolário herdado do §13.2: casar resposta com pergunta é fato apurável em código (existe pendência aberta para este ator?), então **não pode virar julgamento do modelo** — foi o motivo de recusar o caminho de "memória de conversa", que devolveria à LLM uma decisão hoje determinística e transformaria erro de vínculo em dado errado silencioso.
**Decisões desta implementação (17/09/2026):**
- **Status:** `aguardando_porcoes`, por simetria com o `aguardando_categoria` que já existia — não um `pendente` genérico, que anteciparia uma generalização sem segundo caso real.
- **Onde guardar o prato:** coluna nova `pensamentos.item_pendente`, não reaproveitamento de `descricao` — explícito para o eval consultar depois, ao custo de uma linha de migração.
- **Janela:** **20 minutos** entre a pergunta e a resposta que a resolve, e o mesmo limite para o job expirar. É uma aposta calibrada, não uma verdade: o incidente real durou ~1 min; janela longa não custa API (o job não chama LLM) e frustra menos quem demora, mas aumenta a chance de o mesmo ator acumular mais de um prato pendente e a quantidade cair no prato errado — em silêncio, que é justamente a falha que esta seção corrige. Considerou-se 10 min (primeira versão) e até 120 min. Revisar com dado de uso: muita pendência expirando indica régua curta demais.
- **Escopo:** só porcionamento. Generalizar para qualquer guardrail que pergunta fica para quando existir um segundo caso real — sem ele, a abstração é especulação.
- **Resposta sem número dentro da janela** (ex.: "2 unidades de 220g e 2 unidades de 160g"): o produto **pergunta de novo já citando o prato** e mantém a pendência viva. Somar as parcelas é aritmética, e aritmética é do código (§13.2) — enquanto a extração não separar as parcelas, o total continua vindo da pessoa.
- **Placeholder do modelo:** o classificador às vezes devolve o texto literal `"<UNKNOWN>"` em vez de omitir um campo ausente (observado em `claude-sonnet-5`, 3/3, e esporadicamente no `claude-haiku-4-5`). Em `item_nome` isso criaria um prato com esse nome e pularia a pendência, então a ingestão descarta campos com esse valor antes de entregá-los ao produto — o trace no Langfuse continua registrando o bruto do modelo.
**Onde:** `intencao-efeitos.js` (ramo `porcionamento` e `acharPendenciaPorcionamento`, com `JANELA_PENDENCIA_MIN = 20`), `expiracao-porcionamento.js` (job novo, mesmo molde de `lembrete.js`, mas de 2 em 2 minutos — a conversa real se resolve ou morre em minutos; marca `expirada` e avisa o grupo com a frase literal *"Ingestão não concluída por falta de porções: {item_pendente}."*), `relato-ingestao.js` (descarte do `"<UNKNOWN>"`), `server.js` (start do job), `public/app.js` (pílulas dos status novos) e `schema.sql` (Fase 7: coluna `item_pendente` + `status` aceitando `aguardando_porcoes` e `expirada`). Implementado, testado contra produção e mergeado em 17/09/2026 ([PR #13](https://github.com/Djegao/alura-produto-ficticio/pull/13)). **Limitação conhecida, aceita:** porção continua sendo unidade intercambiável — o prato entra com o número certo de porções, sem o peso de cada uma (ver §12).
...
- **Porção é unidade intercambiável, sem peso** (§11.7): `portions_total`/`portions_remaining` contam porções, e o schema não guarda quanto pesa cada uma. "4 de 250 g + 1 de 330 g" vira "5 porções", e "comi uma porção" decrementa 1 seja qual for. Importa quando entrar cálculo calórico por porção. Caminho provável: a extração separar as parcelas (quantidade + peso) e o código somar — hoje a soma vem da pessoa, porque aritmética não é do modelo (§13.2).
