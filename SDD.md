...
### 11.7 Processo correto, saída vazia — o agente consulta tudo e não decide
**Observação:** Mediador disparado com contexto real rico (nota fiscal recém-processada com itens de risoto, um desejo de fim de semana, um lembrete sobre marmitas de lasanha sobrando, e histórico de 3 refeições fora recentes) devolveu `text: ""` e `proposta: {}` — nada.
**Diagnóstico:** confirmado no Langfuse, trace `3d78e29ce5a343d66c5781692dce2748`. O modelo chamou as 5 tools de consulta corretamente numa única primeira chamada (`consultar_estoque`, `consultar_validade_estoque`, `consultar_preferencias`, `consultar_orcamento_semanal`, `consultar_relatos_recentes`). Na segunda chamada, o único bloco de saída foi um `thinking` — sem texto, sem `tool_use` — e `stop_reason` não foi `tool_use`, então o loop principal encerrou (mesmo `break` do §11.4, `agent.js`). A chamada forçada seguinte (`tool_choice: registrar_decisao_cardapio`) obedeceu à obrigação de chamar a ferramenta, mas preencheu o argumento como objeto vazio — nem `proposta` (campo obrigatório no schema da tool) veio preenchido.
**Restrição derivada:** formato de falha diferente do §11.4 (que é truncamento por `max_tokens` no meio de uma tool call). Aqui o processo de consulta foi perfeito — todas as tools certas, na ordem certa — mas o modelo não convergiu numa decisão quando o cenário ficou complexo o bastante (múltiplos sinais concorrentes: desejo novo, tensão de consumo pendente, orçamento, preferências, histórico de refeições fora). Reforça que "seguiu o processo certo" e "produziu resultado útil" são verificações **separadas** — um pipeline de evals precisa checar as duas, nunca assumir que uma garante a outra.
**Cuidado pedagógico:** é tentador narrar isso como "o agente travou pensando na decisão difícil" — funciona como gancho de audiência, mas só é honesto se ficar claro que é uma leitura humana de quem observa, não um estado emocional do modelo. O fato técnico é: processo correto, saída vazia. A interpretação é do instrutor, não do produto.
**Status:** achado preservado como conteúdo real de aula (Aula 2/3), não corrigido — reproduzido ao vivo durante ensaio de gravação.
